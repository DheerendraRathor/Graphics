...
public:
  ...
  //Pause turtle for _t milliseconds
  void pause(const double _t);

  //Repeat the execution of the commands given
  //in _replist _n times.
  void repeat(const unsigned int &_n, const turtle_com_list_t &_replist, GLFWwindow* _window);

  //Executes the command com
  void exec(turtle_com_t *com, GLFWwindow* _window);
}; 
